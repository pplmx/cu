...
__global__ void histogramKernel(const uint8_t* input, uint32_t* histogram,
...
__global__ void histogramPerChannelKernel(const uint8_t* input,
                                          uint32_t* histogram_r,
                                          uint32_t* histogram_g,
                                          uint32_t* histogram_b,
                                          size_t width, size_t height) {
...
void computeHistogram(const uint8_t* d_input, uint32_t* d_histogram,
                      size_t width, size_t height, int bins) {
...
void computeHistogramPerChannel(const uint8_t* d_input,
    ...
    histogramPerChannelKernel<<<grid, block>>>(d_input, d_histogram_r,
...
__global__ void histogramEqualizeKernel(const uint8_t* input, uint8_t* output,
...
void equalizeHistogram(const uint8_t* d_input, uint8_t* d_output,
    ...
    histogramEqualizeKernel<<<grid, block>>>(d_input, d_output, d_histogram,
                                             width, height, scale);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
}
