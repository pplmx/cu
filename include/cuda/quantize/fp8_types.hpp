...
#define NOVA_HD __host__ __device__
#else
#define NOVA_HD
#endif
...
public:
    ...
    NOVA_HD FP8E4M3() : value(0) {}

    NOVA_HD explicit FP8E4M3(float f) : value(float_to_fp8(f)) {}

    NOVA_HD explicit operator float() const { return fp8_to_float(value); }

    NOVA_HD static FP8E4M3 from_bits(uint8_t bits) {
        FP8E4M3 result;
        result.value = bits;
        return result;
    }

    NOVA_HD uint8_t to_bits() const { return value; }
...
private:
    ...
    NOVA_HD static float fp8_to_float(uint8_t bits) {
...
public:
    ...
    NOVA_HD FP8E5M2() : value(0) {}

    NOVA_HD explicit FP8E5M2(float f) : value(float_to_fp8(f)) {}

    NOVA_HD explicit operator float() const { return fp8_to_float(value); }

    NOVA_HD static FP8E5M2 from_bits(uint8_t bits) {
        FP8E5M2 result;
        result.value = bits;
        return result;
    }
